...
class Cell:
    ...
    def __post_init__(self):
        ...
        * (l_i * (e_m - e_o) + l_m * (e_i - 2.0*e_m + e_o) - l_o * (e_i - e_m))) # is this truly a multiply, or a cross?
        ...
        # self.a_1 = 3 * e_i * e_e * sub1 + 3 * e_m * e_e * sub2
        # self.a_2 = 3 * (l_i * e_e + l_e * e_i) * sub1 +  \
        #                 3 * (l_m * e_e + l_e * e_m) * sub2
        # self.a_3 = 3 * l_i * l_e * sub1 + 3 * l_m * l_e * sub2
        # DISREGARD - G() in kot 2000 is F() normalized by R/d, which explains the discrepancy
        # ###
