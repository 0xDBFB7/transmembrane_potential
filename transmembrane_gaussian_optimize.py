...
def cost_function(x):
    ideal_values = normalized_gaussian_pulse(t-(tstop/2),x*1e-9)
    print(np.max(convolve_output(ideal_values, host_cell, dt) * 1e6)/np.max(convolve_output(ideal_values, virus, dt) * 1e6))
    return np.max(convolve_output(ideal_values, host_cell, dt) * 1e6)/np.max(convolve_output(ideal_values, virus, dt) * 1e6)

x0 = [0.1]
x = minimize(cost_function, x0, method="BFGS", options={"disp":True}).x
