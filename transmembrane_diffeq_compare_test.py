...
alpha_v = m.Const((U0 / (T0**2))*(virus.R*virus.a_1/virus.b_1))
beta_v = m.Const((U0 / T0)*(virus.R*virus.a_2/virus.b_1))
gamma_v = m.Const((virus.R*virus.a_3/virus.b_1))
phi_v = m.Const((X0 / T0)*(virus.b_2/virus.b_1))
xi_v = m.Const(X0*(virus.b_3/virus.b_1))
...
m.Equation(x2_v==(alpha_v*u2 + beta_v*u1 + gamma_v*SU0*u0 - phi_v*x1_v - xi_v*x0_v)/(SX0 / (ST0**2)))
...
plt.plot(t1, shift(virus.step_response.astype('float32',casting='same_kind'), nt//2,cval=0.0),marker='o')
